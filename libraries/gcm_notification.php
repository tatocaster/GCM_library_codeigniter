...
class GCM_Notification
{
    ...
    private function _sendCurl($curlPostFields)
    {
        if (!defined('API_ACCESS_KEY')) {
            throw new InvalidArgumentException('API_ACCESS_KEY not defined!');
            return false;
        }
        if (!defined('GCM_URL')) {
            throw new InvalidArgumentException('GCM_URL not defined!');
            return false;
        }
    ...
    private function _debug()
    {
        ...
        if ('200' == $this->_resInfo['http_code']) {
            $response     = explode("\n", $this->_result);
        ...
        } else if ('400' == $this->_resInfo['http_code']) {
            return $responseArray['http_code'] = $this->_resInfo['http_code'];
        } else if ('401' == $this->_resInfo['http_code']) {
            return $responseArray['http_code'] = $this->_resInfo['http_code'];
        } else if ('500' == $this->_resInfo['http_code']) {
            return $responseArray['http_code'] = $this->_resInfo['http_code'];
        }
